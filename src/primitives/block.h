...
public:
    ...
    // Gapcoin PoW fields (hybrid consensus - prime gap mining)
    uint32_t nShift;           // 2^shift multiplier for prime calculation (range: 14-65536)
    uint256 nAdder;            // Offset added to create prime candidate: p = sha256(header) * 2^shift + adder
    uint32_t nGapSize;         // Size of the prime gap found (distance between consecutive primes)
    ...
    // Gapcoin hybrid consensus: check if block has valid prime gap PoW
    bool HasGapcoinProof() const
    {
        return nShift > 0 && nGapSize > 0;
    }

    // Check if block satisfies hybrid consensus (both PoW and PoS required)
    bool IsHybridConsensusComplete() const
    {
        return HasGapcoinProof() && IsProofOfStake();
    }
