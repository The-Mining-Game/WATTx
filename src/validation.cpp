...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // Default: 8333333 satoshis = 0.083333333 WATTx (~50 WATTx per 10 min at 1s blocks)
...
bool Chainstate::UpdateHashProof(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    // HYBRID CONSENSUS: After activation height, require BOTH Gapcoin PoW AND PoS
    if (nHeight >= consensusParams.nHybridConsensusActivationHeight) {
        // Validate Gapcoin prime gap PoW
        if (block.HasGapcoinProof()) {
            std::string strError;
            if (!CheckGapcoinProof(block, consensusParams, strError)) {
                return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "bad-gapcoin-pow",
                    strprintf("UpdateHashProof() : invalid Gapcoin PoW: %s", strError));
            }
        } else {
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "missing-gapcoin-pow",
                strprintf("UpdateHashProof() : missing Gapcoin PoW at height %d", nHeight));
        }

        // Also require PoS (validator signature)
        if (!block.IsProofOfStake()) {
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "missing-pos",
...
bool ChainstateManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, CBlockIndex** ppindex, bool min_pow_checked)
{
    ...
    if (hash != GetConsensus().hashGenesisBlock) {
        ...
        if (nHeight >= consensusParams.nHybridConsensusActivationHeight) {
            // After hybrid activation, require BOTH Gapcoin PoW AND PoS
            if (!block.HasGapcoinProof()) {
                return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "missing-gapcoin-pow",
                    strprintf("missing Gapcoin PoW at height %d", nHeight));
            }
            if (!block.IsProofOfStake()) {
                return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "missing-pos",
                    strprintf("missing PoS validator signature at height %d", nHeight));
            }
            // Validate the Gapcoin proof
            std::string strError;
            if (!CheckGapcoinProof(block, consensusParams, strError)) {
                return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "bad-gapcoin-pow",
...
bool ChainstateManager::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool min_pow_checked)
{
    ...
    if (nHeight >= consensusParams.nHybridConsensusActivationHeight) {
        ...
        if (!block.HasGapcoinProof()) {
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "missing-gapcoin-pow",
                strprintf("%s: missing Gapcoin PoW at height %d", __func__, nHeight));
        }
        if (!block.IsProofOfStake()) {
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "missing-pos",
